 ...
 */
    ...
    class public_categoria_public_proyectosPage extends DetailPage
    {
    ...
    class public_categoriaPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
        ...
        protected function AddFieldColumns(Grid $grid, $withDetails = true)
        {
            if (GetCurrentUserPermissionSetForDataSource('public.categoria.public.proyectos')->HasViewGrant() && $withDetails)
            {
            //
            // View column for public_categoria_public_proyectos detail
            //
            $column = new DetailColumn(array('id_categoria'), 'public.categoria.public.proyectos', 'public_categoria_public_proyectos_handler', $this->dataset, 'Proyectos');
            $column->setMinimalVisibility(ColumnVisibility::PHONE);
            $grid->AddViewColumn($column);
            }
        ...
        protected function doRegisterHandlers() {
            $detailPage = new public_categoria_public_proyectosPage('public_categoria_public_proyectos', $this, array('id_categoria'), array('id_categoria'), $this->GetForeignKeyFields(), $this->CreateMasterDetailRecordGrid(), $this->dataset, GetCurrentUserPermissionSetForDataSource('public.categoria.public.proyectos'), 'UTF-8');
            $detailPage->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource('public.categoria.public.proyectos'));
            $detailPage->SetTitle('Proyectos');
            $detailPage->SetMenuLabel('Proyectos');
            $detailPage->SetHeader(GetPagesHeader());
            $detailPage->SetFooter(GetPagesFooter());
            $detailPage->SetHttpHandlerName('public_categoria_public_proyectos_handler');
            $handler = new PageHTTPHandler('public_categoria_public_proyectos_handler', $detailPage);
            GetApplication()->RegisterHTTPHandler($handler);
        }
    ...
    try
    {
        $Page = new public_categoriaPage("public_categoria", "categoria.php", GetCurrentUserPermissionSetForDataSource("public.categoria"), 'UTF-8');
        $Page->SetTitle('Categoría');
        $Page->SetMenuLabel('Categoría');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("public.categoria"));
